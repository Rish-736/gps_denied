...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        # Softened for smooth 2.0m-corridor flight: with the planner routing paths
        # down corridor centres (nav2 inflation 0.40), repulsion only needs to
        # nudge when the drone is GENUINELY near a wall, so influence is low
        # (no constant both-walls push that made it dart side to side) and the
        # max shift is small. repulsion_max MUST stay well under the corridor
        # half-width (1.0 m here) or a push off one wall overshoots into the other.
        DeclareLaunchArgument('robot_radius', default_value='0.40',
                              description='True x500 prop-tip radius the repulsion protects (m)'),
        DeclareLaunchArgument('repulsion_influence', default_value='0.6',
                              description='Start pushing when a wall is closer than this (m, from prop tips)'),
        DeclareLaunchArgument('repulsion_gain', default_value='0.4',
        ...
        DeclareLaunchArgument('setpoint_lpf', default_value='0.25',
